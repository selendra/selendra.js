...
[ETH-RPC adapter](https://github.com/selendra/selendra.js/tree/master/packages/eth-rpc-adapter), which is
...
noticed another network called `selendraCI`. This network is present within the tutorials, so
that we can verify the expected behaviour of the Selendra EVM+ and you don't need to include it
...
```json
    ...
    "test-selendra": "hardhat test test/HelloWorld.js --network selendra",
    "test-selendra:pubDev": "hardhat test test/HelloWorld.js --network selendraPubDev"
```

As you can see, the `test-selendra` and `test-selendra:pubDev` script differ from `test` script in
the `--network` flag which we use to tell Hardhat to use the `selendra` or `selendraPubDev` network
...
`deploy-selendra` and `deploy-selendra:pubDev` scripts. Once again, we are adding three scripts,
...
```json
    ...
    "deploy-selendra": "hardhat run scripts/deploy.js --network selendra",
    "deploy-selendra:pubDev": "hardhat run scripts/deploy.js --network selendraPubDev",
```
...
which can be run using `yarn test`, `yarn test-selendra` or `yarn test-selendra:pubDev`.
Additionally we added the deploy script, that can be run using `yarn deploy`, `yarn deploy-selendra`
or `yarn deploy-selendra:pubDev`.
