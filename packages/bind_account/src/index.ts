...
export async function bindaccount(substrateMnenonic: string, privateKey: string, rpcProverider: string) {
    ...
    const balance:number = await evm_balances(rpcProverider, evmAddress);
