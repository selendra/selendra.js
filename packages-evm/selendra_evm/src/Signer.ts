...
import { SignerProvider } from '@selendra/eth-providers';
import { handleTxResponse } from '@selendra/eth-providers/lib';
...
export class Signer extends Abstractsigner implements TypedDataSigner {
  ...
  async claimEvmAccount(evmAddress: string): Promise<void> {
    ...
    const data = 'selendra evm:' + Buffer.from(publicKey).toString('hex');
