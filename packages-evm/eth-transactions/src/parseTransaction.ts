...
import { SelendraEvmTX, UnsignedSelendraEvmTX } from './types';
...
function _parseEip712Signature(
  tx: SelendraEvmTX,
  fields: Array<string>,
  serialize: (tx: UnsignedSelendraEvmTX) => string
): void {
  try {
...
export type SignatureType = 'Ethereum' | 'SelendraEip712' | 'Eip1559';

// rlp([chainId, salt, nonce, gasLimit, storageLimit, to, value, data, validUntil, tip, accessList, eip712sig])
export function parseEip712(payload: Uint8Array): SelendraEvmTX {
...
export function parseTransaction(rawTransaction: BytesLike): SelendraEvmTX {
...
export function checkSignatureType(rawTransaction: BytesLike): SignatureType {
  ...
  if (payload[0] === 96) return 'SelendraEip712'; // Selendra EIP-712
