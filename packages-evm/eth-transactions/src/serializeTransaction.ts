...
export function serializeEip712(transaction: UnsignedSelendraEvmTX, signature?: SignatureLike): string {
  const fields: any = [
...
export function serializeTransaction(transaction: UnsignedSelendraEvmTX, signature?: SignatureLike): string {
  ...
  if (transaction.type === 96) {
    return serializeEip712(transaction as UnsignedSelendraEvmTX, signature);
  }
